...
def get_true_classes(filenames):
    ...
    #reurn 1 if there is defective, 0 if the image is great
    y_true = [1 if "good" not in filename.split("\\") else 0 for filename in filenames]
    #y_true = [0 if "hi"  in filename.split("\\") else 1 for filename in filenames]
    return y_true
...
def main(args):
    ...
    for subdir in subdirs:
        ...
        # print precentage
        array_of_correct = np.array(y_true) == np.array(y_pred)
        true_sum=sum(array_of_correct)
        res= true_sum / len(y_pred)
        print("test results in %: ",int(res*100))
